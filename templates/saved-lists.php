 ...
 *
 * This template can be overridden by copying it to yourtheme/wccontour/saved-lists.php.
 *
...
// handle language diversity on account page.
if ( $current_lang && ! $all_account_lang && ! $ajax ) {
	if ( $current_lang !== $default_lang ) {
		$total_saved_lists = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$lists_table} lt LEFT JOIN {$config_table} ct ON (lt.shortcode_id=ct.id) WHERE lt.user_id=%d AND ct.lang=%s", $user_id, $current_lang ) );

	} else {
		$total_saved_lists = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$lists_table} lt LEFT JOIN {$config_table} ct ON (lt.shortcode_id=ct.id) WHERE lt.user_id=%d AND (ct.lang=%s OR ct.lang='')", $user_id, $current_lang ) );
	}
}
// handle language diversity on ajax.
...
// show all lists on account page.
if ( $current_lang && ! $all_account_lang && ! $ajax ) {
	if ( $current_lang !== $default_lang ) {
		$saved_lists = $wpdb->get_results( $wpdb->prepare( "SELECT lt.* FROM {$lists_table} lt LEFT JOIN {$config_table} ct ON (lt.shortcode_id=ct.id) WHERE lt.user_id=%d AND ct.lang=%s LIMIT %d OFFSET %d", $user_id, $current_lang, $lists_per_page, $offset ), ARRAY_A );
	} else {
		$saved_lists = $wpdb->get_results( $wpdb->prepare( "SELECT lt.* FROM {$lists_table} lt LEFT JOIN {$config_table} ct ON (lt.shortcode_id=ct.id) WHERE lt.user_id=%d AND (ct.lang=%s OR ct.lang='') LIMIT %d OFFSET %d", $user_id, $current_lang, $lists_per_page, $offset ), ARRAY_A );
...
<article class="wccon-saved-list" data-id="<?php echo esc_attr( $list['id'] ); ?>">
	<div class="saved-lists__header">
		...
		?>
		<?php if ( ! $ajax ) : ?>
			<button class="wccon-remove-list wccon-icon-button" data-id="<?php echo esc_attr( $list['id'] ); ?>" title="<?php esc_attr_e( 'Delete', 'wccontour' ); ?>"><svg><use xlink:href="#kitrml"></use></svg></button>
			<?php endif; ?>
	</div>
...
endforeach;
	...
	<div class="saved-lists__footer">
		<div class="saved-lists__share">
			...
			<a href="<?php echo esc_url( wccon_get_edit_account_link( $list['id'] ) ); ?>" title="<?php esc_attr_e( 'Edit', 'wccontour' ); ?>" class="saved-lists__button-edit wccon-list-button-edit">
	...
	<div class="saved-lists__share-box">
		<?php
		$socials_links = wccon_get_social_links( $page_id, $list['id'], 'user' );
...
<?php if ( ! $content_exists ) : ?>
	<div class="wccon-no-lists">
		<p><?php echo wp_kses_post( apply_filters( 'wccon_no_lists_found_text', esc_html__( 'No list saved yet', 'wccontour' ) ) ); ?></p>
	</div>
	<?php endif; ?>
</div>
<?php
if ( $ajax && $total_pages > $current_page ) {
	echo '<div class="wccon-button-container"><button class="wccon-load-more" data-current="' . esc_attr( $current_page ) . '" data-type="user" data-total="' . esc_attr( $total_pages ) . '">' . wp_kses_post( apply_filters( 'wccon_load_more_button_text', esc_html__( 'Load more', 'wccontour' ) ) ) . '</button></div>';
} else {
...
<?php
if ( ! $ajax ) {
	wc_get_template( 'icons.php', array(), 'wccontour', WCCON_PLUGIN_PATH . '/templates/' );
}
?>
